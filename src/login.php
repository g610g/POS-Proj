...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-200 via-white to-red-200">
    ...
    <?php
        // show session error flash if present
        use App\Session;
        $loginError = '';

        if (Session::has('error')) {
            $loginError = htmlspecialchars(Session::get('error'));
            Session::unset('error');
        }
        if (Session::has('mismatch')) {
            $loginError = htmlspecialchars(Session::get('mismatch'));
            Session::unset('mismatch');
        }
    ?>

    <!-- Top-center toast for login errors -->
    <div id="login-error-toast" class="fixed left-1/2 top-6 transform -translate-x-1/2 z-50 px-4 pointer-events-none w-[400px]" style="display: <?php echo $loginError ? 'block' : 'none'; ?>;">
        <div class="mx-auto w-full max-w-md pointer-events-auto">
            ...
            <div class="p-4 flex items-start">
                ...
                <div class="ml-3 w-0 flex-1 pt-0.5">
                    ...
                    <p class="mt-1 text-sm text-gray-500" id="login-error-message"><?php echo $loginError; ?></p>
                </div>
                <div class="ml-4 flex-shrink-0 flex">
                    <button id="login-error-close" class="inline-flex text-gray-400 hover:text-gray-600 focus:outline-none">
